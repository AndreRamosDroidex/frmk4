# on_cloud/transform/transformer.py
import os
...
class Transformer:
    ...
    def _load_sql_text(self, sql_or_filename: str, ttype: Optional[str]) -> str:
    ...
    def run(self, df_input: DataFrame, only_ids: Optional[Set[str]] = None) -> Tuple[Dict[str, DataFrame], DataFrame]:
        name_function = get_name_function()
        transforms = self.config._process.get_module("transform")

        #Si no hay transforms en el JSON
        if not transforms:
            return {}, df_input

        #Si pidieron explicitamente skip total (no hay loads que referencien)
        if only_ids is not None and len(only_ids) == 0:
            self.log.registrar("INFO", f"[{name_function}] - Transform SKIPPED (ningún load referencia transform_id).")
            return {}, df_input
        ...
        for i, tcfg in enumerate(transforms, start=1):
            ...
            # Si se especificó subconjunto, y este id no está, lo saltamos
            if only_ids is not None and t_id not in only_ids:
                continue
