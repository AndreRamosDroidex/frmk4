...
def _render_url(tpl: str, kv: Dict[str, str]) -> str:
    """Reemplaza tokens {{key}} en la URL de conexión con valores del dict."""
    url = tpl or ""
...
class LoadPostgres:
    ...
    Espera en load_properties (config-process):
    connection_name opcional, default: 'postgres-cnx-01')
    dbtable            obligatorio 
    mode               opcio     
    select_cols        opcio    

    Además,el run() recibe:
    fecinfo,batch_id-columnas (tipadas) junto a load_ts
    """
